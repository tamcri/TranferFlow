...
import { ShoppingBag, ArrowRightLeft, Clock, MapPin, CheckCircle, Truck, Trash2, Tag } from 'lucide-react';
...
export const ItemCard: React.FC<ItemCardProps> = ({ item, currentStoreId, onRequestTransfer, onConfirmReceipt, onWithdrawItem }) => {
  ...
  const getStatusBadge = () => {
    switch(item.status) {
      case ItemStatus.AVAILABLE:
        return <span className="px-3 py-1 rounded-full text-xs font-bold bg-green-100 text-green-700">DISPONIBILE</span>;
      case ItemStatus.PENDING:
        return <span className="px-3 py-1 rounded-full text-xs font-bold bg-yellow-100 text-yellow-700 flex items-center"><Truck size={12} className="mr-1"/> IN TRANSITO</span>;
      case ItemStatus.TRANSFERRED:
        return <span className="px-3 py-1 rounded-full text-xs font-bold bg-slate-100 text-slate-600">RICEVUTO</span>;
    }
  };

  return (
    <div className={`bg-white rounded-xl shadow-sm border border-slate-200 p-5 transition-all hover:shadow-md flex flex-col justify-between ${isTransferred ? 'opacity-60 grayscale-[0.5]' : ''}`}>
      <div>
          ...
          <div className="flex items-center space-x-2">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] font-bold uppercase tracking-wider bg-slate-100 text-slate-500 px-1.5 py-0.5 rounded border border-slate-200">{item.gender}</span>
              </div>
      ...
      <div className="mt-2 pt-4 border-t border-slate-100">
        {/* Scenario: È roba mia che ho caricato io */}
        {isMySourceItem && (
           <div className="space-y-2">
             <div className="w-full py-2 px-4 bg-slate-50 text-slate-500 rounded-lg text-sm font-medium border border-slate-100 text-center">
               {isPending ? `Richiesto da ${item.destinationStoreName}` : isTransferred ? 'Trasferimento Completato' : 'Il tuo articolo'}
             </div>
             {/* Pulsante Ritira merce se è ancora disponibile */}
             {isAvailable && (
               <button 
        ...
        {/* Scenario: Sono io che devo ricevere la merce (Conferma Ricezione) */}
        ...
        {/* Scenario: È roba mia ed è già arrivata */}
        ...
        {/* Scenario: È roba di altri e posso richiederla */}
